"""
METHOD 4: SegFormer-B5 fine-tuning
...
class SegDataset(Dataset):
    def __init__(self, img_dir, mask_dir, augment=True):
        ...
        self.processor = SegformerImageProcessor(do_resize=True, size={'h': 512, 'w': 512})
        ...
        print(f"  SegDataset: {len(self.ids)} samples")
...
def train():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        scheduler.step()
        val_iou = validate(model, test_dl)
    ...
    print(f"\nDone. Best IoU: {best_iou:.4f}  Checkpoint: {CKPT_OUT}")
