"""
MedSAM Fine-Tuning — Simple & OOM-proof
GPU: trains mask decoder only (encoder frozen)

SETUP (run once):
  pip install git+https://github.com/facebookresearch/segment-anything.git
  curl -L -o checkpoints/medsam/medsam_vit_b.pth https://huggingface.co/bowang-lab/MedSAM/resolve/main/medsam_vit_b.pth
...
IMG_SIZE  = 1024   # SAM requires exactly 1024x1024
EPOCHS    = 50
LR        = 2e-4
...
class BurnDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # bbox prompt from mask (or full image if no burn)
...
def train():
    ...
    if not os.path.exists(CKPT_IN):
        print(f"\nERROR: MedSAM checkpoint not found at:\n  {CKPT_IN}")
        print("\nDownload it with:")
        print(f"  curl -L -o {CKPT_IN} https://huggingface.co/bowang-lab/MedSAM/resolve/main/medsam_vit_b.pth")
        return

    try:
        from segment_anything import sam_model_registry
    except ImportError:
        print("\nERROR: segment-anything not installed. Run:")
        print("  pip install git+https://github.com/facebookresearch/segment-anything.git")
        return

    print(f"Device : {DEVICE}")
    print(f"Loading MedSAM from {CKPT_IN} ...")
    model = sam_model_registry['vit_b'](checkpoint=CKPT_IN).to(DEVICE)

    # freeze encoder — only train decoder + prompt encoder
    ...
    model.image_encoder.gradient_checkpointing = True  # saves VRAM
    ...
    for epoch in range(1, EPOCHS+1):
        model.train()
        model.image_encoder.eval()  # keep encoder in eval mode
        total_loss = 0.0
    ...
    model.load_state_dict(torch.load(CKPT_OUT))
    model.eval()
    test_dice = []
    with torch.no_grad():
    ...
    print(f"Test Dice: {np.mean(test_dice):.4f}  ({len(test_dice)} images)")
